...
float ndf(vec3 lm, vec3 n) {
    ...
    return step(0.0, cosTheta) * alpha2 / (M_PI * pow(cosTheta, 4) * pow((alpha2 + tanTheta2), 2));
}
...
float g1(vec3 v, vec3 m, vec3 n) {
    ...
    float tanTheta2 = pow(cosTheta, -2) - 1;
    return step(0.0, dot(v, m) / dot(v, n)) * 2 / (1 + sqrt(1 + alpha2 * tanTheta2));
}
...
void main() {
    ...
    for (int i = 0; i < POINT_LIGHTS_NUM; i++) {
        ...
        if (dot(lightDir, normal) > 0 && dot(viewDir, normal) > 0)
            color = color + evalMicrofacet(viewDir, lightDir, normal) * light.radiance;
    }
